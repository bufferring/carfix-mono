...
const ProductForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setServerError(error.response?.data?.error || error.message || 'Failed to fetch product');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const validateImage = (file) => {
    if (!ALLOWED_TYPES.includes(file.type)) return 'Invalid file type. Only JPEG, PNG and GIF are allowed.';
  ...
  const validateForm = () => {
    ...
    if (!product.stock || parseInt(product.stock, 10) < 0) errors.push('Valid stock quantity is required');
    if (errors.length > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setServerError(error.response?.data?.error || error.message || 'Failed to save product');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" data-aos="fade-up" data-aos-delay="100">
        ...
        <div className="flex justify-end mt-6">
          <button type="submit" disabled={isSaving || isLoading} className={`inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white ${isSaving || isLoading ? 'bg-red-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500'}`}>
            {isLoading ? 'Loading...' : isSaving ? 'Saving...' : 'Save Product'}
          </button>
        </div>
      </form>
    </div>
